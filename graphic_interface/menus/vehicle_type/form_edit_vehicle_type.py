...
from backend.exceptions.vehicle_type_exceptions import VehicleTypeCreateException, VehicleTypeFormatDataException, \
    VehicleTypeAlreadyExistsException, VehicleTypeMissingMandatoryDataException
...
class FormEditVehicleType(BaseFrame):
    ...
    def __init__(self, root, connection, master, values):
        ...
        frame_1 = tk.LabelFrame(self.root, text="Editar Tipo de Vehículo", bg="black", foreground="white", font='bold')
        ...
        button_1 = tk.Button(frame_3, text="Guardar", font='Helvetica 15 bold', width=15, bg="gold2", command=self.edit_vehicle_type)
    ...
    def edit_vehicle_type(self):
        ...
        try:
            self.vehicle_type.update(
                vehicle_type_id=id,
                brand=brand,
                model=model,
                year=year
            )
            self.show_info(message="El tipo de vehículo ha sido registrado exitosamente")
        except VehicleTypeCreateException:
            self.show_error(
                message="ERROR: El tipo de vehículo no ha sido creado. Por favor verifique que todos los datos estan completos")
            ...
            self.show_error(message=
                            "ERROR: Faltan algunos datos obligatorios."
